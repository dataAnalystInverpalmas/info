...
use App\Models\ProyectoLogro;

class ProyectoLogroController {
    private const JSON_FLAGS = JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES;

    public static function listar($proyecto_id) {
        $usuario_id = $_SESSION['id'] ?? null;
        return ProyectoLogro::getByProyecto((int)$proyecto_id, $usuario_id);
    }

    public static function obtener($id) {
        $usuario_id = $_SESSION['id'] ?? null;
        return ProyectoLogro::getById((int)$id, $usuario_id);
    }
    ...
    public static function crear() {
        ...
        $impacto = trim((string)($input['impacto'] ?? ''));
        $fecha_logro = $input['fecha_logro'] ?? null;
        ...
        $estados_validos = ['registrado', 'validado'];
        if (!in_array($estado, $estados_validos, true)) {
            $estado = 'registrado';
        }

        $usuario_id = $_SESSION['id'] ?? null;
        return ProyectoLogro::create($proyecto_id, $descripcion, $impacto, $fecha_logro, $estado, $autor, $usuario_id);
    }
    ...
    public static function actualizar($id) {
        ...
        $usuario_id = $_SESSION['id'] ?? null;
        return ProyectoLogro::update((int)$id, $descripcion, $impacto, $fecha_logro, $estado, $autor, $usuario_id);
    }

    public static function eliminar($id) {
        $usuario_id = $_SESSION['id'] ?? null;
        return ProyectoLogro::delete((int)$id, $usuario_id);
    }
