        ...
        <div id="filtersColF" class="col-sm-3">
            <div class="card-panel filtersf">
                ...
                if (isset($conexion) && $conexion) {
                    $qr = $conexion->query("SELECT DISTINCT programa FROM programf ORDER BY programa DESC");
                    if ($qr) {
                        while ($r = $qr->fetch_object()) { $programas[] = $r->programa; }
                        $qr->free();
                    }
                    $qr = $conexion->query("SELECT DISTINCT variedad FROM programf WHERE variedad IS NOT NULL AND variedad <> '' ORDER BY variedad");
                    if ($qr) {
                        while ($r = $qr->fetch_object()) { $variedades[] = $r->variedad; }
                        $qr->free();
                    }
                    $qr = $conexion->query("SELECT DISTINCT temporada_obj FROM programf WHERE temporada_obj IS NOT NULL AND temporada_obj <> '' ORDER BY temporada_obj");
                    if ($qr) {
                        while ($r = $qr->fetch_object()) { $temporadas[] = $r->temporada_obj; }
                        $qr->free();
                    }
                    $qr = $conexion->query("SELECT DISTINCT producto FROM programf WHERE producto IS NOT NULL AND producto <> '' ORDER BY producto");
                    if ($qr) {
                        while ($r = $qr->fetch_object()) { $productos[] = $r->producto; }
                        $qr->free();
                    }
                    $qr = $conexion->query("SELECT DISTINCT finca FROM programf WHERE finca IS NOT NULL AND finca <> '' ORDER BY finca");
                    if ($qr) {
                        while ($r = $qr->fetch_object()) { $fincas[] = $r->finca; }
                        $qr->free();
                    }
                    $qr = $conexion->query("SELECT DISTINCT bloque FROM programf WHERE bloque IS NOT NULL ORDER BY bloque");
                    if ($qr) {
                        while ($r = $qr->fetch_object()) { $bloques[] = $r->bloque; }
                        $qr->free();
                    }
                }
                ?>
