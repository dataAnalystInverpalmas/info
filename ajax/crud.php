...
$where = " WHERE estado = 1 AND raiz=0 AND fecha_ensarte BETWEEN '$fecha_ini' AND '$fecha_fin' ";

switch($opcion){
    case 1:
        $consulta = "INSERT INTO $tabla (variedad, temporada_obj, fecha_ensarte, fecha_ensarte_r, esquejes_ensarte, banco) VALUES('$variedad', '$temporada_obj', '$fecha_ensarte_r', '$fecha_ensarte_r', $esquejes_ensarte, '$banco') ";			
    ...
    case 2:
        if ((isset($fecha_ensarte_r) and isset($esquejes_ensarte)) and empty($fecha_cosecha_r)){        
            $consulta = "UPDATE $tabla SET fecha_ensarte_r='$fecha_ensarte_r', esquejes_ensarte='$esquejes_ensarte', banco='$banco' WHERE id='$id' ";		
            $resultado = $conexion->prepare($consulta);
            $resultado->execute();        
        }else if (isset($fecha_ensarte_r) and isset($esquejes_ensarte) and isset($fecha_cosecha_r) and isset($esquejes_cosecha) ){
            $consulta = "UPDATE $tabla SET fecha_ensarte_r='$fecha_ensarte_r', esquejes_ensarte='$esquejes_ensarte', fecha_cosecha_r='$fecha_cosecha_r', esquejes_cosecha='$esquejes_cosecha', banco='$banco' WHERE id='$id' ";		
            $resultado = $conexion->prepare($consulta);
            $resultado->execute();
        }
    ...
    case 3:        
        ...
        break;
    case 4:    
        $consulta = "SELECT id,variedad,temporada_obj,fecha_ensarte_r,esquejes_ensarte,fecha_cosecha_r,esquejes_cosecha,banco,plantas FROM $tabla $where ";
        $resultado = $conexion->prepare($consulta);
        $resultado->execute();        
        $data=$resultado->fetchAll(PDO::FETCH_ASSOC);
        break;
