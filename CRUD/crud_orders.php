...
$evaluador = (isset($_POST['evaluador'])) ? $_POST['evaluador'] : '';
...
$sql = "SELECT id FROM evaluators WHERE nombre='$evaluador' LIMIT 1";
$result = $conexion->query($sql); 
$row = $result->fetch_assoc(); 
$evaluador_id = $row['id'];

$sql = "SELECT id FROM varieties WHERE nombre='$variedad' LIMIT 1";
$result = $conexion->query($sql); 
$row = $result->fetch_assoc(); 
$variedad_id = $row['id'];
...
switch($opcion){
    case 1:
        ...
        $consulta = "INSERT INTO $tabla (date, evaluator_id, user_id) VALUES('$fecha', $evaluador_id, $user_id) ";			
    ...
    case 3:        
        ...
        break;
    case 4:
        $consulta = "SELECT a.id,a.date as fecha,b.nombre as evaluador FROM $tabla as a
                    INNER JOIN evaluators as b ON b.id=a.evaluator_id $where";
        $resultado = $conn->prepare($consulta);
        $resultado->execute();        
        $data=$resultado->fetchAll(PDO::FETCH_ASSOC);
        break;
